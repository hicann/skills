...
| 技能名称 | 用途 |
|---------|------|
| ascendc-custom-op-to-kernel-launch | 内置算子直调改造模板 |
...
查看 `agents/` 目录下的 Agent 实现：
- `ascendc-ops-architect` - 算子架构师
- `ascendc-ops-reviewer` - 算子代码检视
...
| 层级 | 目录 | 用途 | 内容示例 |
|------|------|------|---------|
| skills | `references/` | 按需加载的知识文档 | API 指南、最佳实践、约束说明 |
| skills | `assets/` | 输出时使用的静态资源 | 模板文件、图标 |
| skills | `scripts/` | 可执行脚本 | 数据生成、验证脚本 |
| teams | `workflows/` | teams 流程配置文件 | 任务提示词、数据流定义、错误处理指南 |
