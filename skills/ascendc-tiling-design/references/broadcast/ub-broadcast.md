# Broadcast - UB Broadcast 分支

> **适用场景**: 合轴后多维，存在需要广播的输入。搬入原始数据 → UB 内用 `Broadcast()` API 扩展 → 计算
...
A2/A3/A5 通用接口，dim 和 axis 为编译期模板参数，仅支持 1D/2D、axis=0 或 1。
...
| **地址重叠** | src 和 dst 不能重叠 |
| **dtype（A2/A3）** | int8_t, uint8_t, half, float |
