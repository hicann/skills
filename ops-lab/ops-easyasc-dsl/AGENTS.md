...
- For kernel authoring tasks:
  ...
  - prefer `agent/scripts/select_kernel_example.py`, `agent/scripts/gen_kernel_skeleton.py`, and `agent/scripts/estimate_matmul_datamove.py` before opening large prose files
  - then read only the specific constraint or pattern file you need
  - if you still need prose example notes, use `agent/references/examples/kernel-index.md` before `agent/references/examples/kernel-catalog.md`
